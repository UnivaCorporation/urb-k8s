...
inline Result<Process> process(pid_t pid)
{
  ...
  Try<Duration> utime = Seconds(pstatus.pr_utime.tv_sec) + Nanoseconds(pstatus.pr_utime.tv_nsec);
  Try<Duration> stime = Seconds(pstatus.pr_stime.tv_sec) + Nanoseconds(pstatus.pr_stime.tv_nsec);
  ...
  return Process(pstatus.pr_pid,
		 ...
		 stime.isSome() ? stime.get() : Option<Duration>::none(),
                 psinfo.pr_fname,
                 (psinfo.pr_nzomb == 0) && (psinfo.pr_nlwp == 0) && (psinfo.pr_lwp.pr_lwpid == 0));
}
