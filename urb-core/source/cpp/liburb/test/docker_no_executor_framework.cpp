...
{
public:
  DockerNoExecutorScheduler(int tasks = TOTAL_TASKS, const string image = "alpine")
    : tasksLaunched(0), tasksFinished(0), totalTasks(tasks), dockerImage(image) {}
                              ...
                              const vector<Offer>& offers)
  {
    ...
    for (size_t i = 0; i < offers.size(); i++) {
      ...
      while (tasksLaunched < totalTasks &&
        ...
        TaskInfo task;
        task.set_name("Docker Task " + lexical_cast<string>(taskId));
        task.mutable_task_id()->set_value(lexical_cast<string>(taskId));
        task.mutable_slave_id()->MergeFrom(offer.slave_id());
        task.mutable_command()->set_value("echo hello_from_docker; ps -ef");
        ...
        dockerInfo.set_image(dockerImage);
...
int main(int argc, char** argv)
{
  ...
  if (argc == 2) {
    ...
    cerr << "Usage: " << argv[0] << " <master> [tasks] [docker_image]" << endl;
    return -1;
  }

  DockerNoExecutorScheduler scheduler(tasks, image.get());
