...
public:
  ...
  virtual ~JNIScheduler() {VLOG(1) << "~JNIScheduler";}
...
{
  VLOG(1) << "JNIScheduler::disconnected";
  jvm->AttachCurrentThread(JNIENV_CAST(&env), nullptr);
...
void JNIScheduler::resourceOffers(SchedulerDriver* driver,
                                  const vector<Offer>& offers)
{
  VLOG(1) << "JNIScheduler::resourceOffers";
/*
  jint version = env->GetVersion();
  VLOG(1) << "JNIScheduler::resourceOffers: version=" << version;
  JNIEnv* lenv;
  jint envret = jvm->GetEnv(JNIENV_CAST(&lenv), version);
  VLOG(1) << "JNIScheduler::resourceOffers: GetEnv ret=" << envret;
  if (envret == JNI_EDETACHED) {
      VLOG(1) << "JNIScheduler::resourceOffers: detached";
  } else if (envret== JNI_EVERSION) {
      VLOG(1) << "JNIScheduler::resourceOffers: wrong version";
  }
  JavaVM *vmBuf[10];
  jsize nVMs;
  jint cr = JNI_GetCreatedJavaVMs(vmBuf, 10, &nVMs);
  VLOG(1) << "JNIScheduler::resourceOffers: JNI_GetCreatedJavaVMs returned " << cr << ", nVMs=" << nVMs;
*/
  jint ret = jvm->AttachCurrentThread(JNIENV_CAST(&env), nullptr);
  VLOG(1) << "JNIScheduler::resourceOffers: after AttachCurrentThread, returned " << ret;
  ...
  env->ExceptionClear();
  VLOG(1) << "JNIScheduler::resourceOffers: before CallVoidMethod";

  env->CallVoidMethod(jscheduler, resourceOffers, jdriver, joffers);
  VLOG(1) << "JNIScheduler::resourceOffers: after CallVoidMethod";

  if (env->ExceptionCheck()) {
    VLOG(1) << "JNIScheduler::resourceOffers: after ExceptionCheck";
    env->ExceptionDescribe();
    env->ExceptionClear();
    VLOG(1) << "JNIScheduler::resourceOffers: before DetachCurrentThread";
    jvm->DetachCurrentThread();
    VLOG(1) << "JNIScheduler::resourceOffers: before abort";
    driver->abort();
    VLOG(1) << "JNIScheduler::resourceOffers: after abort";
    return;
  }

  VLOG(1) << "JNIScheduler::resourceOffers: before DetachCurrentThread";
  jvm->DetachCurrentThread();
  VLOG(1) << "JNIScheduler::resourceOffers: end";
}
