  ...
  void received(queue<Event> events)
  {
    while (!events.empty()) {
      ...
      switch (event.type()) {
        ...
        case Event::MESSAGE: {
          cout << endl << "Received a MESSAGE event: " << event.message().data() << endl;
          break;
        }
...
//      taskResources.allocate(framework.role());
      ...
      while (tasksLaunched < totalTasks &&
        ...
        cout << "Preparing launching task " << taskId << " using offer "
             << offer.id() << endl;
...
//        task.mutable_command()->set_value("sleep 1");
	  ...
	  cout << "Launching task " << taskId << " using offer " << offer.id() << endl;
    ...
    if (status.state() == TASK_FINISHED) {
      ++tasksFinished;
        ...
        if (status.state() == TASK_RUNNING) {
          cout << "Sending message to task" << status.task_id() << endl;
          Call call;
          CHECK(framework.has_id());
          call.mutable_framework_id()->CopyFrom(framework.id());
          call.set_type(Call::MESSAGE);

          Call::Message* msg = call.mutable_message();
          msg->mutable_agent_id()->CopyFrom(status.agent_id());
          msg->mutable_executor_id()->CopyFrom(status.executor_id());
          msg->set_data("message from framework to executor");
          
          mesos->send(call);            
        
          if (tasksFinished%2) {
            cout << "Sending kill to task" << status.task_id() << endl;
            Call call;
            CHECK(framework.has_id());
            call.mutable_framework_id()->CopyFrom(framework.id());
...
//        status.state() == TASK_KILLED ||
  ...
  void finalize()
  {
    ...
    mesos->send(call);
    cout << "In finalize, TEARDOWN sent" << endl;
    sleep(1);
  }
